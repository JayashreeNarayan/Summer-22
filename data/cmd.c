...
char usage[] = " Use: errors [-opt] file\n\
 Reads in file and prints out average, error and autocorrelation time\n\
  ...
  m m          : get value of a parameter passed, say alpha\n\
...
int main(int argc, char *argv[])
{
  ...
  double alpha = 0;   /*  to get value of alpha, inv of dtau = 10^{alpha}} */
  int i, opt;
  char *fname; /* data file name */

  /* Process all command line arguments and filename */
  //get_mu = (argc > 3) ? atof(argv[3]) : 0;
  FILE *Result = fopen("Result.txt", "a"); // write only
  ...
  while (opt = cmdl_get_opt())
  {
    ...
    {
    case 'm':
      alpha = cmdl_get_double(usage);
      break;
    case 't':
      autoc = 0;
      break;
    case 'T':
      autof = cmdl_get_int(usage);
      break;
    case 'n':
      nmeas = cmdl_get_int(usage);
      break;
    case 's':
      skip = cmdl_get_int(usage);
      break;
    case 'b':
      block = cmdl_get_int(usage);
      break;
    case 'c':
      do
      {
  ...
  printf("Value of the parameter passed i.e. alpha is: %g\n", alpha);

  if (ncols == 0)
    err_args();                   /* col has to be given */
  ...
  for (i = 0; i < ncols; i++)
  {
    ...
    else
    {
      ...
      if (block)
      {
        ...
        printf("block size %d\n",block);
        if (i == 0)
          printf(" col - average - sigma\n");
        fprintf(Result, " %.10g  %.8g %d %d\n", ave, sig, block, n/block);
        //printf(" %d  %.10g  %.8g\n",col[i],ave,sig);
      }
      else
      {
        ...
        if (i == 0)
          printf("  average - sigma \n");
        fprintf(Result, "  %.10g  %.8g %g\n", ave, sig,tint);
        //printf(" %d  %.10g  %.8g  %g\n",col[i],ave,sig,tint);
      }
    }
  }
               ...
               double *avep, double *sigp, double *tintp)
{
  ...
  if (autocorr)
  {
    ...
    if (it >= n / 2)
      fprintf(stderr, " ** correlation > N/2*%d\n", TINTSTOP);
  }
...
void blockerr(double d[], int n, int blocksize, double *avep, double *sigp)
{
  double ave, sig, *blockave, *jackave, *jave;
  int i, id, iblock, nblocks, l;

  ave = sig = 0.0;
  nblocks = n / blocksize;
  blockave = (double *)malloc(nblocks * sizeof(double));
  jackave = (double *)malloc(nblocks * sizeof(double));
  jave = (double *)malloc(nblocks * sizeof(double));

  id = 0;
  for (iblock = 0; iblock < nblocks; iblock++)
  {
    blockave[iblock] = 0;
    for (i = 0; i < blocksize; i++, id++)
      blockave[iblock] += d[id];
  ...
  for (iblock = 0; iblock < nblocks; iblock++){
    ...
    jave[iblock] = 0;
    for(l = 0; l < nblocks; l++){
    if(l != iblock){jackave[iblock] += blockave[l];}
    }
    jave[iblock] = jackave[iblock]/(nblocks-1);}
    for (iblock = 0; iblock < nblocks; iblock++){
    ave = ave + jave[iblock];}
  ave /= nblocks;
  for (iblock = 0; iblock < nblocks; iblock++){
    sig += sqr(jave[iblock] - ave);
   //printf("%.8g\n",sig);
}
  sig = sqrt((nblocks - 1)*sig / (nblocks));
